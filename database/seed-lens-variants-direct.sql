...
from catalog.products;
, '', 'g'), '.', ''), '-PLAIN-', replace(upper(product.size), ' ', ''), '-', product.duration_code, '-', product.rate_code) as sku_code,
    power.power_sign,
    power.power_value,
    'Plain' as color_name,
    product.size,
  ...
  select
    pg_temp.seed_uuid('sku:' || product.name || ':' || power.power_sign || ':' || power.power_value || ':' || color.color_name || ':Pack 2') as id,
    product.id as product_id,
    concat('CV-CL-', case when power.power_sign = '-' then 'M' else 'P' end, replace(regexp_replace(power.power_value::text, '\.0+$|(?<=\.\d)0$', '', 'g'), '.', ''), '-', color.color_code, '-PACK2') as sku_code,
    power.power_sign,
    power.power_value,
    color.color_name,
    'Pack 2',
...
select
  ...
  count(*) filter (where product_type = 'Lens' and is_active and name in ('Clear Vision Colored Medical Lens Pack', 'Plain Medical Lens Box', 'Plain Medical Lens Vial')) as active_generic_lens_products
from catalog.products;
, '', 'g'), '.', ''), '-', color.color_code, '-PACK2', '-', product.duration_code, '-', product.rate_code) as sku_code,
    power.power_sign,
    power.power_value,
    color.color_name,
    'Pack 2',
...
, '', 'g'), '.', ''), '-PLAIN-', replace(upper(product.size), ' ', ''), '-', product.duration_code, '-', product.rate_code) as sku_code,
  ...
  select
    pg_temp.seed_uuid('sku:' || product.name || ':' || power.power_sign || ':' || power.power_value || ':' || color.color_name || ':Pack 2') as id,
    product.id as product_id,
    concat('CV-CL-', case when power.power_sign = '-' then 'M' else 'P' end, replace(regexp_replace(power.power_value::text, '\.0+$|(?<=\.\d)0
    power.power_sign,
    power.power_value,
    color.color_name,
    'Pack 2',
